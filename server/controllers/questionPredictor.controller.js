...
export const predictQuestions = async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ message: "No PDF file uploaded" })

    const user = await UserModel.findById(req.userId)
    if (!user) return res.status(404).json({ message: "User not found" })
    if (user.credits < 5) return res.status(403).json({ message: "Insufficient credits. Question prediction costs 5 credits." })
    ...
    try {
      ...
      console.log(`✅ Predictor PDF: ${numPages} pages, ${pdfText.length} chars`)
    } catch (err) {
    ...
    const prompt = buildQuestionPredictorPrompt(trimmedText)
    ...
    console.error("Question Predictor Error:", error)
    res.status(500).json({ error: "Question prediction failed", message: error.message })
  }
}
